<div class="p-6 border-t border-gray-200 dark:border-gray-700 md:border-l">
    <form wire:submit.prevent="submit" class="flex flex-col" action="post">
        <label for="name" class="block">
            <span class="text-gray-700">Name</span>
            <input class="block w-full mt-1 form-input" wire:model="name" placeholder="Enter Your Name" id="name" type="text" autocomplete="off" required>
        </label>
        @error('name') <span class="mt-1 ml-1 text-sm text-red-700">{{ $message }}</span> @enderror

        <label for="email" class="block mt-4">
            <span class="text-gray-700">Email</span>
            <input class="block w-full mt-1 form-input" wire:model="email" placeholder="Enter Your E-mail" id="email" autocomplete="off" type="email" required>
        </label>
        ...
        <label for="phone" class="hidden sm:block mt-4">
            ...
            <input class="block w-full mt-1 form-input" wire:model="phone" placeholder="Enter Your Mobile No." id="phone" autocomplete="off" type="tel">
        </label>
        ...
        <label class="block mt-4">
            ...
            <textarea class="block w-full mt-1 form-textarea" wire:model="message" rows="3"
                placeholder="Write Your Little Message."></textarea>
        </label>
        @error('message') <span class="mt-1 ml-1 text-sm text-red-700">{{ $message }}</span> @enderror

        <button class="px-4 py-2 mt-8 font-semibold text-gray-800 bg-white border border-gray-300 rounded shadow hover:bg-gray-100" style="background-color: #28a745; color: #fff; border-color: #28a745;">
            Send Your Message
        </button>
    </form>
</div>